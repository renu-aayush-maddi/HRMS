...
public class SalaryStructureController : ControllerBase
{
    ...
    [HttpPost]
    public IActionResult Create(CreateSalaryStructureDto dto)
    {
        salaryStructureService.Create(dto);

        return Ok("Salary Structure Created Successfully");
    }

    [HttpGet]
    public IActionResult GetAll()
    {
        return Ok(salaryStructureService.GetAll());
    }

    [HttpGet("{id}")]
    public IActionResult GetById(Guid id)
    {
        return Ok(salaryStructureService.GetById(id));
    }

    [Authorize(Roles = "Admin,HR")]
    [HttpPut("{id}")]
    public IActionResult Update(Guid id, UpdateSalaryStructureDto dto)
    {
        salaryStructureService.Update(id, dto);
